import SelectHinhThuc from '@/pages/DaoTaoV2/DanhMucHeThong/Bo/HinhThuc/components/SelectHinhThuc';
...
const FormTrinhDo = (props: any) => {
	...
	const { record, setVisibleForm, edit, postModel, putModel, formSubmiting, visibleForm } = useModel(
		'daotaov2.danhmuc.hinhthucdaotao',
	);
	const { title } = props;
		...
		<Card title={`${edit ? 'Chỉnh sửa' : 'Thêm mới'} ${title?.toLowerCase()}`}>
			<Form onFinish={onFinish} form={form} layout='vertical'>
				<Row gutter={[12, 0]} style={{ marginBottom: 12 }}>
					<Col xs={24}>
						<Form.Item name='maDmHinhThuc' label='Hình thức đào tạo của Bộ GD&ĐT' rules={[...rules.required]}>
							<SelectHinhThuc
								selectMa
					...
					<Col span={24} md={8}>
						<Form.Item name='ma' label='Mã' rules={[...rules.required, ...rules.length(20)]}>
							<Input placeholder='Nhập mã hình thức' disabled={edit} />
						</Form.Item>
					</Col>
					<Col span={24} md={16}>
						<Form.Item name='ten' label='Tên hình thức' rules={[...rules.required, ...rules.length(250)]}>
