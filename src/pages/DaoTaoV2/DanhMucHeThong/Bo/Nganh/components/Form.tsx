...
const FormTrinhDo = (props: any) => {
	...
	const { record, setVisibleForm, edit, postModel, putModel, formSubmiting, visibleForm } =
		useModel('daotaov2.danhmuc.dmnganh');
	const { title } = props;
		...
		<Card title={`${edit ? 'Chỉnh sửa' : 'Thêm mới'} ${title?.toLowerCase()}`}>
			<Form onFinish={onFinish} form={form} layout='vertical'>
				<Row gutter={[12, 0]} style={{ marginBottom: 12 }}>
					...
					<Col xs={24}>
						<Form.Item name='maDmNhomNganh' label='Nhóm ngành đào tạo' rules={[...rules.required]}>
							<SelectNhomNganh selectMa />
						</Form.Item>
					</Col>
					<Col xs={24}>
						<Form.Item name='maDmTrinhDo' label='Trình độ đào tạo' rules={[...rules.required]}>
